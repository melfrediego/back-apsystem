<div>
    <div class="card shadow-sm mb-5 mr-2 pb-8 mb-xl-4">
        ...
        <div class="card-header border-0 pt-0">
            <h3 class="card-title align-items-start flex-column">
                <span class="card-label fw-semibolder fs-4 mb-0">Categorias</span>
                <span class="text-muted mt-1 fw-normal fs-7">listagem de categorias</span>
            </h3>
        ...
        <div class="row px-8 mb-4">
            <div class="col-sm-12 col-md-12 d-flex align-items-center justify-content-center justify-content-md-start">
                <div class="d-flex align-items-center">
                    ...
                    <select wire:model='destaqueFiltro' wire:change="resetPage" class="form-select form-select-sm form-select-solid w-120px">
                            ...
                            @foreach ($categorias as $categoria)
                                <tr>
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <div class="symbol symbol-50px me-5">
                                                @if ($categoria->logo)
                                                    <img src="{{ asset('storage/' . $categoria->logo) }}" alt="Logo" width="50">
                                                @else
                                                    Sem logo
                                                @endif
                                            </div>
                                    ...
                                    <td class="text-end min-w-150px">
                                        <a href="{{ route('categorias.edit', ['id' => $categoria->id]) }}" class="btn btn-icon btn-light-primary btn-color-primary btn-sm me-1">
                                            <i class="fa-solid fa-info"></i>
                                        </a>
                                        <a href="{{ route('categorias.edit', ['id' => $categoria->id]) }}" class="btn btn-icon btn-light-warning btn-color-warning btn-sm me-1">
                                            <i class="fa-solid fa-pen-to-square"></i>
                                        </a>
                                        @if ($btnDelete)
                                            {{-- <form action="{{ route('marcas.destroy', $categoria->id) }}" method="POST" style="display: inline;">
                                                @csrf
                                                @method('DELETE')
    ...
    <div class="row">
        ...
        <div class="col-sm-12 col-md-4 d-flex align-items-center justify-content-center justify-content-md-start">
            ...
            <span class="px-4">Mostrando {{ $categorias->firstItem() }} a {{ $categorias->lastItem() }} de {{ $categorias->total() }} registros.</span> 
        ...
        <div class="col-sm-12 col-md-8 d-flex align-items-center justify-content-center justify-content-md-end">
            {{ $categorias->links('vendor.livewire.custom-pagination'); }}
        </div>
    </div>
